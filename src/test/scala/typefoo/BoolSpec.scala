...
class BoolSpec extends BaseSpec {
  ...
  implicit def toAssociativeOr[A <: Bool, B <: Bool, C <: Bool](implicit ev: ||[A, ||[B, C]] =:= ||[||[A, B], C]): AssociativityOr[A, B, C] = new AssociativityOr[A, B, C]

  class AssociativityAnd[A, B, C]
  implicit def toAssociativeAnd[A <: Bool, B <: Bool, C <: Bool](implicit ev: &&[A, &&[B, C]] =:= &&[&&[A, B], C]): AssociativityAnd[A, B, C] = new AssociativityAnd[A, B, C]

  class DistributivityAndOverOr[A, B, C]
  implicit def toDistributivityAndOverOr[A <: Bool, B <: Bool, C <: Bool](implicit ev: &&[A, ||[B, C]] =:= ||[&&[A, B], &&[A, C]]): DistributivityAndOverOr[A, B, C] = new DistributivityAndOverOr[A, B, C]

  class DistributivityOrOverAnd[A, B, C]
  implicit def toDistributivityOrOverAnd[A <: Bool, B <: Bool, C <: Bool](implicit ev: ||[A, &&[B, C]] =:= &&[||[A, B], ||[A, C]]): DistributivityOrOverAnd[A, B, C] = new DistributivityOrOverAnd[A, B, C]

  def ternaryLaws[A <: Bool, B <: Bool, C <: Bool](implicit
    ev0: AssociativityOr[A, B, C],
    ev1: AssociativityAnd[A, B, C],
    ev2: DistributivityAndOverOr[A, B, C],
    ev3: DistributivityOrOverAnd[A, B, C]) = true
  ...
  implicit def toCommutativeOr[A <: Bool, B <: Bool](implicit ev: ||[A, B] =:= ||[B, A]): CommutativityOr[A, B] = new CommutativityOr[A, B]

  class CommutativityAnd[A, B]
  implicit def toCommutativeAnd[A <: Bool, B <: Bool](implicit ev: &&[A, B] =:= &&[B, A]): CommutativityAnd[A, B] = new CommutativityAnd[A, B]

  class Absorption1[A, B]
  implicit def toAbsorption1[A <: Bool, B <: Bool](implicit ev: &&[A, ||[A, B]] =:= A): Absorption1[A, B] = new Absorption1[A, B]

  class Absorption2[A, B]
  implicit def toAbsorption2[A <: Bool, B <: Bool](implicit ev: ||[A, &&[A, B]] =:= A): Absorption2[A, B] = new Absorption2[A, B]

  class DeMorgan1[A, B]
  implicit def toDeMorgan1[A <: Bool, B <: Bool](implicit ev: &&[Not[A], Not[B]] =:= Not[A || B]): DeMorgan1[A, B] = new DeMorgan1[A, B]

  class Demorgan2[A, B]
  implicit def toDeMorgan2[A <: Bool, B <: Bool](implicit ev: ||[Not[A], Not[B]] =:= Not[A && B]): Demorgan2[A, B] = new Demorgan2[A, B]

  def binaryLaws[A <: Bool, B <: Bool](implicit
    ev0: CommutativityOr[A, B],
    ev1: CommutativityAnd[A, B],
    ev2: Absorption1[A, B],
    ev3: Absorption2[A, B],
    ev4: DeMorgan1[A, B],
    ev5: Demorgan2[A, B]) = true
